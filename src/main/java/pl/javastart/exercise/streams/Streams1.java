...
public class Streams1 {
    ...
    public List<Book> convertToBooks(List<String> titles) {
        return titles.stream().map(title -> new Book(title)).collect(Collectors.toList());
    }
    ...
    public List<Book> convertToBooksAndReturnOnlyStartingWithGra(List<String> titles) {
        return titles.stream().filter(s -> s.startsWith("Gra")).map(title -> new Book(title)).collect(Collectors.toList());
    }
}
